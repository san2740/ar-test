...
uniform int typhoonCenterCount;
uniform vec4 typhoonCenters[10];
...
out vec2 v_lonLat;
out float v_typhoonBoost;

float wrapLongitude180(float lon) {
...
// 추가: 경위도 거리(도 단위 근사)
float lonLatDistDeg(vec2 a, vec2 b) {
    float dLon = wrapLongitude180(a.x - b.x);
    float dLat = a.y - b.y;
    float x = dLon * cos(radians((a.y + b.y) * 0.5));
    return length(vec2(x, dLat));
}

// 추가: 태풍 강조값 계산
float getTyphoonBoost(vec2 lonLat) {
    float boost = 0.0;

    for (int i = 0; i < 10; i++) {
        if (i >= typhoonCenterCount) break;

        vec4 c = typhoonCenters[i];
        float radiusDeg = max(c.z, 0.0001);
        float strength = max(c.w, 0.0);

        float dist = lonLatDistDeg(lonLat, c.xy);

        // 중심 가까울수록 1, 반경 바깥은 0
        float w = 1.0 - smoothstep(0.0, radiusDeg, dist);

        // 가장 강한 중심 하나 기준으로 적용
        boost = max(boost, w * strength);
    }
...
void main() {
    ...
    float tLen = max(float(trailLength - 1), 1.0);
    trailFade = 1.0 - (float(trailIndex) / tLen);
    ...
    v_lonLat = lonlatalt.xy;
    v_typhoonBoost = getTyphoonBoost(v_lonLat);
