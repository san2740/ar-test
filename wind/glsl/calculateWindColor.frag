...
void main() {
    ...
    if(colorMode == 0) {
        ...
        // 습도
        // normalizedPosition = vec3(0.5);
        // vec3 positionRate = (currentPosition - minimum) / (maximum - minimum);
        // float relativeHumidity;
        // if(positionRate.z < 0.5) {
        //     float low = texture(colors[0], positionRate.xy).r;
        //     float mid = texture(colors[1], positionRate.xy).r;
        //     relativeHumidity = mix(low, mid, positionRate.z * 2.0);
        // } else {
        //     float mid = texture(colors[1], positionRate.xy).r;
        //     float high = texture(colors[2], positionRate.xy).r;
        //     relativeHumidity = mix(mid, high, (positionRate.z - 0.5) * 2.0);
        // }
    ...
    fragColor_1 = vec4(color, 0.8);

    // color 
    // fragColor_1 = vec4(vec3(normalizeSpeed), normalizeSpeed);
    // fragColor_1 = vec4(vec3(1.0), 0.8);
}
